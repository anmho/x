...
const OMNICHANNEL_ITEMS = [
  ...
  { href: '/api-keys', label: 'Manage API Keys', icon: Key, section: 'apiKeys' as NavSection },
...
const MAIN_ITEMS = [
  { href: '/cron-jobs', label: 'Cron Jobs', section: 'cronJobs' as NavSection, icon: Calendar },
  { href: '/workflows', label: 'Workflows', section: 'workflows' as NavSection, icon: Workflow },
...
const CMD_ITEMS = [
  ...
  { href: '/notifications/create', label: 'Send Notification', section: 'Pages' },
...
const PAGE_TITLES: Partial<Record<NavSection, { title: string; description: string }>> = {
  ...
  omnichannel: { title: 'Omnichannel', description: 'Send notifications, manage API keys, monitor status, and run campaigns.' },
  ...
  apiKeys: { title: 'API Key Management', description: 'Mint and revoke scoped application keys.' },
...
function readStoredProjectId() {
  if (typeof window === 'undefined') return CATALOG_PROJECTS[0]?.id ?? '';
  const stored = window.localStorage.getItem('console:selected-project');
  return stored && CATALOG_PROJECTS.some((p) => p.id === stored) ? stored : CATALOG_PROJECTS[0]?.id ?? '';
}
        ...
        <div className="fixed inset-0 z-50">
          ...
          <div className="relative mx-auto mt-[12vh] max-w-xl px-4">
            <Command
              ...
              className={`overflow-hidden rounded-xl border border-zinc-700 bg-zinc-950 shadow-2xl transition-all duration-200 ${cmdOpen ? 'translate-y-0 scale-100 opacity-100' : '-translate-y-2 scale-[0.98] opacity-0'}`}
            >
