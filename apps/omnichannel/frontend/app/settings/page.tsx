...
type OAuthServiceClientStatus = 'active' | 'revoked';
...
type OAuthServiceClient = {
  ...
  scopes: string[];
  status: OAuthServiceClientStatus;
  createdAt: string;
};

const OAUTH_SERVICE_CLIENTS_KEY = 'console:oauth:service-clients:v1';
...
function readOAuthServiceClients(): OAuthServiceClient[] {
  if (typeof window === 'undefined') return [];
  try {
    const raw = window.localStorage.getItem(OAUTH_SERVICE_CLIENTS_KEY);
    if (!raw) return [];
    const parsed = JSON.parse(raw);
    if (!Array.isArray(parsed)) return [];
    return parsed as OAuthServiceClient[];
  } catch {
    return [];
  }
}

function writeOAuthServiceClients(clients: OAuthServiceClient[]) {
  if (typeof window === 'undefined') return;
  window.localStorage.setItem(OAUTH_SERVICE_CLIENTS_KEY, JSON.stringify(clients));
}

function readInitialSettingsTab(): Tab {
  if (typeof window === 'undefined') return 'general';
  const params = new URLSearchParams(window.location.search);
  const t = params.get('tab');
  return t === 'integrations' || t === 'security' || t === 'general' ? t : 'general';
}

function parseOAuthScopes(value: string): string[] {
  return Array.from(new Set(value.split(/[\s,]+/).map((entry) => entry.trim()).filter(Boolean)));
}

function maskClientCredential(value: string): string {
  if (!value) return '';
...
function makeCredentialID(prefix: string): string {
  const randomPart =
    typeof crypto !== 'undefined' && typeof crypto.randomUUID === 'function'
      ? crypto.randomUUID().replace(/-/g, '').slice(0, 8)
      : `${Date.now().toString(36)}${(credentialCounter++).toString(36)}`;
  return `${prefix}_${randomPart}`;
}

export default function SettingsPage() {
  const [tab, setTab] = useState<Tab>(readInitialSettingsTab);
...
function ServiceToServiceOAuthSection() {
  const [clients, setClients] = useState<OAuthServiceClient[]>(readOAuthServiceClients);
  const [name, setName] = useState('');
  const [clientID, setClientID] = useState('');
  const [clientCredentials, setClientCredentials] = useState('');
  const [scopes, setScopes] = useState('domains:read domains:write');
  const [error, setError] = useState<string | null>(null);
  const [success, setSuccess] = useState<string | null>(null);

  useEffect(() => {
    writeOAuthServiceClients(clients);
  }, [clients]);

  function createClient(event: FormEvent) {
    event.preventDefault();
    const trimmedName = name.trim();
    const trimmedClientID = clientID.trim();
    const trimmedCredentials = clientCredentials.trim();
    const parsedScopes = parseOAuthScopes(scopes);

    if (!trimmedName || !trimmedClientID || !trimmedCredentials) {
      setError('name, client_id, and client_credentials are required');
      setSuccess(null);
      return;
    }
    ...
    const duplicate = clients.some((entry) => entry.clientId === trimmedClientID && entry.status === 'active');
    if (duplicate) {
      setError(`active client_id "${trimmedClientID}" already exists`);
      setSuccess(null);
      return;
    }

    const created: OAuthServiceClient = {
      id: makeCredentialID('oauth'),
      name: trimmedName,
      clientId: trimmedClientID,
      clientCredentials: trimmedCredentials,
      grantType: 'client_credentials',
      scopes: parsedScopes,
      status: 'active',
    ...
    setScopes(parsedScopes.join(' '));
    setError(null);
    setSuccess(`Created OAuth client "${trimmedName}"`);
  }

  function revokeClient(clientId: string) {
    setClients((prev) => prev.map((entry) => (entry.id === clientId ? { ...entry, status: 'revoked' } : entry)));
    setSuccess('Client revoked');
    setError(null);
  }

  function rotateCredentials(clientId: string) {
    const rotated = makeCredentialID('cc');
    setClients((prev) =>
      prev.map((entry) =>
        entry.id === clientId
          ? {
              ...entry,
              clientCredentials: rotated,
              status: 'active',
            }
          : entry,
      ),
    );
    setSuccess('Client credentials rotated');
    setError(null);
  }

  return (
    <section className="rounded-xl border border-zinc-800 bg-zinc-950 p-5">
      <h2 className="mb-1 text-sm font-semibold text-zinc-200">OAuth Service-to-Service</h2>
      <p className="text-sm text-zinc-400">
        Manage machine-to-machine clients with grant type <code className="rounded bg-zinc-900 px-1 py-0.5 text-zinc-300">client_credentials</code>.
      </p>

      {error && <p className="mt-3 rounded-md border border-rose-500/30 bg-rose-500/10 px-3 py-2 text-sm text-rose-300">{error}</p>}
      {success && <p className="mt-3 rounded-md border border-emerald-500/30 bg-emerald-500/10 px-3 py-2 text-sm text-emerald-300">{success}</p>}

      <form onSubmit={createClient} className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2">
        <label className="block">
          <span className="mb-1 block text-xs text-zinc-500">Name</span>
          <input
            value={name}
            onChange={(event) => setName(event.target.value)}
            placeholder="notifications-worker"
            className="w-full rounded-md border border-zinc-700 bg-zinc-900 px-3 py-2 text-sm text-zinc-100"
          />
        </label>
        <label className="block">
          <span className="mb-1 block text-xs text-zinc-500">client_id</span>
          <input
            value={clientID}
            onChange={(event) => setClientID(event.target.value)}
            placeholder="notif-worker-client"
            className="w-full rounded-md border border-zinc-700 bg-zinc-900 px-3 py-2 font-mono text-sm text-zinc-100"
          />
        </label>
        <label className="block md:col-span-2">
          <span className="mb-1 block text-xs text-zinc-500">client_credentials</span>
          <input
            value={clientCredentials}
            onChange={(event) => setClientCredentials(event.target.value)}
            placeholder="cc_..."
            className="w-full rounded-md border border-zinc-700 bg-zinc-900 px-3 py-2 font-mono text-sm text-zinc-100"
          />
        </label>
        <label className="block md:col-span-2">
          <span className="mb-1 block text-xs text-zinc-500">Scopes (space/comma separated)</span>
          <input
            value={scopes}
            onChange={(event) => setScopes(event.target.value)}
            placeholder="domains:read domains:write"
            className="w-full rounded-md border border-zinc-700 bg-zinc-900 px-3 py-2 font-mono text-sm text-zinc-100"
          />
        </label>
      ...
      <div className="mt-4 space-y-3">
        {clients.length === 0 ? (
          <p className="text-sm text-zinc-500">No OAuth clients configured.</p>
        ) : (
          clients.map((client) => (
            <article key={client.id} className="rounded-lg border border-zinc-800 bg-zinc-900/30 p-3">
              <div className="flex items-start justify-between gap-2">
                <div>
                  <p className="text-sm font-medium text-zinc-100">{client.name}</p>
                  <p className="mt-0.5 text-xs text-zinc-400">
                    <span className="font-mono">client_id={client.clientId}</span> • grant={client.grantType}
                  </p>
                  <p className="mt-0.5 text-xs text-zinc-500">created {new Date(client.createdAt).toLocaleString()}</p>
                </div>
                <span
                  className={`rounded-full px-2 py-0.5 text-xs ${
                    client.status === 'active' ? 'bg-emerald-500/20 text-emerald-300' : 'bg-zinc-700 text-zinc-300'
                  }`}
                >
                  {client.status}
                </span>
              ...
              <p className="mt-2 text-xs text-zinc-400">
                client_credentials: <code className="rounded bg-zinc-900 px-1 py-0.5 font-mono">{maskClientCredential(client.clientCredentials)}</code>
              </p>
              <div className="mt-2 flex flex-wrap gap-1">
                {client.scopes.map((scope) => (
                  <span key={`${client.id}:${scope}`} className="rounded-full border border-zinc-700 bg-zinc-900 px-2 py-0.5 text-xs text-zinc-300">
                    {scope}
                  </span>
                ))}
              </div>
              <div className="mt-3 flex gap-2">
                <button
                  type="button"
                  onClick={() => rotateCredentials(client.id)}
                ...
                {client.status === 'active' && (
                  <button
                    ...
                    className="rounded-md border border-zinc-700 bg-zinc-900 px-2 py-1 text-xs text-rose-300 hover:bg-zinc-800"
                  >
                    Revoke client
                  </button>
                )}
